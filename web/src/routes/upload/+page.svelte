<script lang="ts">
	...
	const createJob = async (jobsUrl: string, mediaId: string) => {
		...
		return (await response.json()) as TrimiaJob;
	};
</script>
...
<main class="min-h-screen bg-[radial-gradient(circle_at_top_left,var(--muted),transparent_34rem)] px-6 py-12">
	<section class="mx-auto flex w-full max-w-3xl flex-col gap-8">
		<div class="space-y-3">
			<p class="text-muted-foreground text-sm font-medium">Signed in as {data.user.email}</p>
			...
			<p class="text-muted-foreground max-w-2xl text-lg text-pretty">
				Send an MP4, MOV, or WebM file to the local Trimia server. Trimia will save the upload
				and probe the duration before the next editing step.
			</p>
		</div>
		...
		<Card.Root class="shadow-sm">
			<Card.Header>
				...
				<Card.Description>Large files are supported. Keep this tab open while Trimia uploads and starts analysis.</Card.Description>
			</Card.Header>
			<Card.Content>
				<form class="space-y-5" onsubmit={uploadVideo}>
					<div class="space-y-2">
						<Label for="file">Video file</Label>
						<Input id="file" name="file" type="file" accept="video/mp4,video/quicktime,video/webm" bind:files />
						<p class="text-muted-foreground text-sm">The browser uploads directly to Trimia with a short-lived upload token, so SvelteKit does not buffer the video.</p>
					</div>
					...
					{#if uploading}
						<div class="space-y-2">
							...
							<p class="text-muted-foreground text-sm">Uploading {progress}%</p>
						</div>
					{/if}
			...
			<Alert>
				...
				<AlertDescription>
					...
					<span class="text-muted-foreground block text-sm">
						Media ID {media.mediaId} · {formatDuration(media.durationSeconds)} · {formatBytes(media.sizeBytes)}
					</span>
				</AlertDescription>
			</Alert>
		{/if}
